...
export function installDevMock(): void {
  const api: Api = {
      ...
      listRedisDBs: async () => [{ index: 0, keys: 0 }],
      redisScan: async () => ({ cursor: 0, keys: [] }),
      redisGetKey: async () => ({ key: '', type: 'string', ttl_seconds: -1 }),
      redisRenameKey: async () => {},
      redisDeleteKeys: async () => 0,
      redisSetTTL: async () => {},
      ...
      redisListPush: async () => {},
      redisListDeleteIndex: async () => {},
      redisSetAdd: async () => {},
      redisSetRemove: async () => {},
      redisZSetAdd: async () => {},
      redisZSetRemove: async () => {},
      redisServerInfo: async () => ({ mode: 'standalone' as const, used_memory_human: '', connected_clients: 0, total_keys: 0 }),
