...
const props = defineProps<{ connectionId: string; database: string; schema: string; relation: string; relationType?: 'table' | 'view' | 'materialized_view' }>()

// 每页行数与后端 PostgresPageRows 的 limit 对齐。
const PAGE_SIZE = 200
...
interface PostgresPageRowsResult {
  columns: PgColumn[]
  rows: (string | null)[][]
  total_rows: number
  primary_key: string[]
...
// 条件(PostgreSQL 语义下后端契约:仅主键列,前端禁止拼 SQL 文本)。
interface PgCellRef {
  column: string
  value: string | null
}
...
// Postgres* 绑定尚未由 wails generate module 生成,这里按显式形状断言调用
// (req 为契约形状原样,Wails 按字段名序列化,snake_case 已对齐);生成后
// 签名一致,无需改动本文件。
const app = App as unknown as {
  PostgresPageRows(req: {
    connection_id: string
    database: string
    schema: string
    relation: string
    ...
    limit: number
    offset: number
  }): Promise<PostgresPageRowsResult>
  PostgresTruncateTable(req: { connection_id: string; database: string; schema: string; relation: string; relation_kind: 'table' | 'view' | 'materialized_view' }): Promise<void>
  PostgresPreviewCellUpdate(req: unknown): Promise<{ statement: string; matched_rows: number }>
  PostgresUpdateCell(req: unknown): Promise<void>
}

const columns = ref<PgColumn[]>([])
...
// 视图与物化视图不允许 TRUNCATE:清空数据入口仅对 table 显示。
const isTable = computed(() => (props.relationType ?? 'table') === 'table')
...
async function fetchPage(): Promise<void> {
  ...
  editing.value = null
  try {
    const res = await app.PostgresPageRows({
...
const truncateMessage = computed(
  () => `确认清空表「${props.schema}.${props.relation}」的全部数据？此操作不可恢复。`,
)

async function doTruncate(): Promise<void> {
  confirmTruncate.value = false
  try {
    await app.PostgresTruncateTable({
      connection_id: props.connectionId,
      database: props.database,
      schema: props.schema,
      relation: props.relation,
      relation_kind: props.relationType ?? 'table',
    })
    offset.value = 0
    await fetchPage()
  } catch (e) {
...
// colTitle 列头悬停提示:排序说明 + 类型。
function colTitle(c: PgColumn): string {
  return `按 ${c.name} 排序 · ${c.type}`
}
...
function buildTarget(ed: { row: number; col: number }): PgCellUpdateTarget {
...
let pending: PgCellUpdateTarget | null = null
...
async function onCellUpdateConfirm(): Promise<void> {
  ...
  try {
    await app.PostgresUpdateCell(target)
    pending = null
    confirmOpen.value = false
    statement.value = ''
    matchedRows.value = 0
...
<template>
  <div class="pg-browser" data-test="pg-table-browser">
    <div class="summary" data-test="pg-summary">
      <span class="mono table-name" data-test="pg-summary-table">{{ props.database }}.{{ props.schema }}.{{ props.relation }}</span>
      <span class="sep">·</span>
      <span data-test="pg-summary-type">{{ props.relationType || 'table' }}</span>
      <span class="sep">·</span>
      <span data-test="pg-summary-rows">≈ {{ totalRows.toLocaleString() }} 行</span>
    </div>
    ...
    <div class="toolbar">
      ...
      />
      <button class="btn ghost" type="button" data-test="btn-pg-apply" :disabled="loading" @click="applyWhere">过滤</button>
      <button class="btn ghost" type="button" data-test="btn-pg-refresh" :disabled="loading" @click="refresh">
        {{ loading ? '加载中…' : '刷新' }}
      </button>
      <button
        v-if="isTable"
        ...
        type="button"
        data-test="btn-pg-truncate"
        @click="confirmTruncate = true"
      >清空数据</button>
    </div>

    <div v-if="error || cuError" class="msg err" data-test="pg-error">{{ error || cuError }}</div>

    <div v-if="columns.length > 0 && rows.length === 0 && !loading" class="table-wrap">
      <div class="fields-panel" data-test="pg-fields-panel">
        <div class="fields-title" data-test="pg-fields-title">表结构(共 {{ columns.length }} 字段)</div>
        <div v-for="c in columns" :key="c.name" class="fields-row" data-test="pg-field-row">
          <span class="field-name" data-test="pg-field-name">{{ c.name }}</span>
          <span class="field-type" data-test="pg-field-type">{{ c.type }}</span>
        </div>
      </div>
      <div class="empty" data-test="pg-grid-empty">
    ...
    <div v-else class="table-wrap">
      <table class="table" data-test="pg-grid">
        <thead>
          <tr>
            <th
              v-for="c in columns"
              ...
              data-test="pg-col"
              tabindex="0"
              :aria-sort="orderBy === c.name ? (asc ? 'ascending' : 'descending') : 'none'"
              :title="colTitle(c)"
              @click="sortBy(c.name)"
              @keydown.enter.prevent="sortBy(c.name)"
              @keydown.space.prevent="sortBy(c.name)"
            >
              <!-- 弹性布局只在内层 div:th 必须保持 table-cell,否则脱离
                   表格列布局,WebKit 会把整行列头堆进一个匿名单元格。 -->
              <div class="col-head">
                <span class="col-name">
                  <span data-test="pg-col-name">{{ c.name }}</span>
                  <!-- 主键角标:后端 is_in_primary_key 标记。 -->
                  <span v-if="c.is_in_primary_key" class="pk-badge" data-test="pg-pk-badge" title="主键">🔑</span>
                  <span v-if="orderBy === c.name" class="col-arrow">{{ asc ? '↑' : '↓' }}</span>
                </span>
                <span class="col-type" data-test="pg-col-type">{{ c.type }}</span>
              </div>
            </th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(row, ri) in rows" :key="ri" data-test="pg-row">
            <td
              ...
              @keydown.enter.prevent="startEdit(ri, ci)"
              @keydown.space.prevent="startEdit(ri, ci)"
            >
          ...
          <tr v-if="rows.length === 0 && !loading">
            <td :colspan="columns.length || 1" class="empty" data-test="pg-grid-empty">
              <div class="empty-icon">◌</div>
              <div>该表暂无数据</div>
              <div class="empty-hint">数据写入后点击「刷新」查看</div>
            </td>
    ...
    <div class="pager" data-test="pg-pager">
      <button class="btn ghost" type="button" data-test="btn-pg-prev" :disabled="offset === 0 || loading" @click="prevPage">上一页</button>
      <span class="page-label" data-test="pg-page-label">第 {{ page }} 页</span>
      <button class="btn ghost" type="button" data-test="btn-pg-next" :disabled="rows.length < PAGE_SIZE || loading" @click="nextPage">下一页</button>
    </div>
...
<style scoped>
.pg-browser { padding: 16px; color: var(--text); font-family: var(--font); }
.summary {
